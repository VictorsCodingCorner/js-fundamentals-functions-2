...
function parseRequest(req) {
  ...
  request.method = requestLines[0].split(' ')[0]
  request.path = requestLines[0].split(' ')[1].split('?')[0]
  ...
  for (const line of requestLines.slice(1)) {
    ...
    parseHeader(trimmedLine, request.headers)
  }

  request.query = extractQuery(requestLines[0].split(' ')[1])
...
function extractQuery(path) {
  if (path.includes('?')) {
    ...
    for (const [key, value] of query.entries()) {
      object[key] =
        object[key] !== undefined ? [].concat(object[key], value) : value
    }
    return object
  }
  return null
